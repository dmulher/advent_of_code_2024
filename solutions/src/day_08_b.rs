    ...
    .map(|(_, positions)| {
      ...
      for pos_a in positions.iter().map(|(x, y)| (*x as i32, *y as i32)) {
        for pos_b in positions.iter().map(|(x, y)| (*x as i32, *y as i32)) {
          ...
          let mut beyond_a = (pos_a.0, pos_a.1);
          while beyond_a.0 >= 0 && beyond_a.0 < height as i32 && beyond_a.1 >= 0 && beyond_a.1 < width as i32 {
            signals.insert((beyond_a.0, beyond_a.1));
            beyond_a = (beyond_a.0 - distance.0, beyond_a.1 - distance.1);
          }
          let mut beyond_b = (pos_b.0, pos_b.1);
          while beyond_b.0 >= 0 && beyond_b.0 < height as i32 && beyond_b.1 >= 0 && beyond_b.1 < width as i32 {
            signals.insert((beyond_b.0, beyond_b.1));
            beyond_b = (beyond_b.0 + distance.0, beyond_b.1 + distance.1);
          }
        }
      }
      signals
...
mod tests {
  ...
  #[test]
  fn test_day_08_b() {
    const EXAMPLE_ANSWER: Option<u16> = Some(34);
    const ANSWER: Option<u16> = Some(951);
