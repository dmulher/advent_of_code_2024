...
fn add_result(total_result: (Result, Result), result: Result) -> (Result, Result) {
  match (total_result, result) {
    ((Result::Ordered(a), Result::Reordered(b)), Result::Ordered(v)) => (Result::Ordered(a+v), Result::Reordered(b)),
    ((Result::Ordered(a), Result::Reordered(b)), Result::Reordered(v)) => (Result::Ordered(a), Result::Reordered(b+v)),
    ((a, b), _) => (a, b),
  }
}
...
fn get_instructions(contents: String) -> u32 {
  ...
  let (_part_a, part_b) = updates_str
    .lines()
    .map(|update| {
      ...
      if commands.is_sorted_by(|a, b| compare_fn(a, b, &rules) == Ordering::Less) {
        Result::Ordered(commands[commands.len()/2].1 as u32)
      } else {
        commands.sort_by(|a, b| compare_fn(a, b, &rules));
        Result::Reordered(commands[commands.len()/2].1 as u32)
      }
    })
    .fold((Result::Ordered(0), Result::Reordered(0)), add_result);

  if let Result::Reordered(val) = part_b {
    val
  } else {
    0
  }
